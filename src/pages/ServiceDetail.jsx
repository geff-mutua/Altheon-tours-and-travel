    ...
    <article className="service-page">
      ...
      {service.example && (
        <section className="section gathering-example">
          <div className="wrap">
            <div className="section-head"><div><span className="eyebrow">Example Programme</span><h2>A three-day hybrid-company gathering</h2></div><p>The company deals with Altheon while Altheon coordinates the suppliers and arrangements.</p></div>
      ...
      <section className="section related-services">
        <div className="wrap">
          <div className="section-head"><div><span className="eyebrow">Connected Solutions</span><h2>Your request may need more than one desk.</h2></div><Link to="/services" className="btn btn-ghost">View all services</Link></div>
          <div className="related-services__grid">
            {related.map((item) => <Link to={`/services/${item.slug}`} key={item.slug}><img src={item.cardImage} alt="" loading="lazy" /><div><span>{item.eyebrow}</span><h3>{item.name}</h3><strong>Explore <ArrowRight size={13} /></strong></div></Link>)}
          </div>
        </div>
      </section>
